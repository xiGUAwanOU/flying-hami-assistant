...
class ReminderCommandHandler {
  ...
  async _handleList (args, chatId, replyToMessageId) {
    ...
    const reminderTexts = reminders.map(reminder => {
      let nextExecutionTime
      if (reminder.nextExecutionTime) {
        nextExecutionTime = moment(reminder.nextExecutionTime).tz(this.config.timeZone).format('YYYY-MM-DD HH:mm')
      } else {
        nextExecutionTime = ''
      }
      return `[${reminder._id}] [${nextExecutionTime}] ${reminder.task} (${reminder.user.slice(1)})`
    })
